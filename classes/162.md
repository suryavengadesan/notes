...
- Simply fairness
	- providing fairness among processes (generalizes to users, applications, threads)
...
2nd Edition by Robert B. Cooper

https://www.cse.fau.edu/~bob/publications/IntroToQueueingTheory_Cooper.pdf

# 1. Scope and Nature of Queueing Theory

- System of systems
	- Occurences and lenghts specified probalistically

- Queueing theory (limited)
	- Congestion theory (more general)
	- Stochastic Service System Theory (even more general)

- Example model
	- Customers request use of equipment (server)
...
- Example model 
	-  Meet prespecified service cirerion
		- Estimate telephone traffic loade between two cities
		- Estimate number of trunks required
	- Derivation
		- Ensure conservation of flow 
		- Defintions
			- \lambda = arrival rate = request/time 
			- \tau = mean holding time = time a request holds a trunk (switch)
			- offered load = a = lambda * tau = arrival rate * mean holding time
				- measured in erlangs (units)

# 2. Review of Topics from Theory of Probability and Stochasitic Processes
...
# 3. Birth and Death Queueing Models
